...
class CRUDRepository:
    ...
    def get_many(
        self, db: Session, *args, order_by = None, page: int = 1, limit: int = 100, **kwargs
    ) -> Page[ORMModel]:
        """
        ...
        Returns:
            Page[ORMModel]: Page of retrieved records.
        """
        ...
        return Page(items=items, page=page, total_results=total_results, total_pages=total_pages)

    
    def pagination_query(self, query , page : int , limit : int):
        """
        Retrieves multiple records from the database as a page

        Parameters:
            query : the sqlalchem query
                query = db.query(MyClass).join(OtherClass, MyClass.id == OtherClass.owner_id)
